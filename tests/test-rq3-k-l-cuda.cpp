// tests/test-rq3-k-l-cuda.cpp
//
// Validates the GGML_TYPE_RQ3 CUDA kernels against the CPU reference backend by
// building the SAME ggml graph on both backends and diffing the float outputs.
// The CPU backend's mul_mat dispatches to ggml_vec_dot_rq3_f32, so a two-backend
// graph compare is exactly the desired CUDA-vs-CPU equivalence.
//
// Three NMSE checks (each <= 1e-4-class vs CPU):
//   (a) VEC_DOT generic MMVQ:  n=1, small K -> vec_dot_rq3_q8_1_rot (vecdotq.cuh)
//   (b) VEC_DOT ncols1:        n=1, large K -> mul_mat_vec_rq3_q8_1_ncols1
//   (c) PREFILL:               n=64         -> dequant+cuBLAS (MMQ not yet wired)
//   (d) DEQUANT via get_rows   -> k_get_rows_rq3 vs CPU + round-trip NMSE
//
// Modeled on tests/test-rq4-cuda.cpp. Public GGML API only.
#include <ggml.h>
...
static constexpr int64_t QK = 256; // == QK_RQ3 == QK_K
...
// Pack a [ncol, nrow] RQ3 tensor on host via the public quantize API.
static std::vector<uint8_t> make_rq3(int64_t nrow, int64_t ncol, uint32_t seed,
                                         std::vector<float> & f32_out)
{
    ...
    size_t row_bytes = (ncol / QK) * ggml_type_size(GGML_TYPE_RQ3);
    std::vector<uint8_t> packed((size_t) nrow * row_bytes);
    size_t w = ggml_quantize_chunk(GGML_TYPE_RQ3, f32_out.data(), packed.data(),
                                   0, nrow, ncol, nullptr);
    GGML_ASSERT(w == (size_t) nrow * row_bytes);
    return packed;
}

// (a/b/c) mul_mat: src0 = rq3 [k, m], src1 = F32 [k, n], dst = [m, n].
...
static ggml_tensor * build_mul_mat(ggml_context * ctx) {
    ...
    ggml_tensor * a   = ggml_new_tensor_2d(ctx, GGML_TYPE_RQ3, c.k, c.m);
...
// (d) DEQUANT via get_rows: src0 = rq3 [ncol, nrow], idx = 0..nrow-1
...
static ggml_tensor * build_get_rows(ggml_context * ctx) {
    ggml_tensor * a   = ggml_new_tensor_2d(ctx, GGML_TYPE_RQ3, g_gr.ncol, g_gr.nrow);
...
int main(void)
{
    ...
    if (!be_cuda) { std::puts("CUDA backend not found; skipping RQ3 CUDA validation"); return 0; }
    ...
    // (a) generic MMVQ: n=1, small K (8 blocks) -> vec_dot_rq3_q8_1_rot
    {
        g_cur_mm = { 16, 1, 8 * QK };
        std::vector<float> f32_a, f32_b((size_t) g_cur_mm.k * g_cur_mm.n);
        auto packed = make_rq3(g_cur_mm.m, g_cur_mm.k, 0xC0FFEEu, f32_a);
        uint32_t s = 0xA11CEu;
    ...
    // (b) ncols1: n=1, large K (>= 8 blocks) -> mul_mat_vec_rq3_q8_1_ncols1
    {
        g_cur_mm = { 16, 1, 64 * QK };
        std::vector<float> f32_a, f32_b((size_t) g_cur_mm.k * g_cur_mm.n);
        auto packed = make_rq3(g_cur_mm.m, g_cur_mm.k, 0x1234u, f32_a);
        uint32_t s = 0x5678u;
        for (auto & v : f32_b) v = frand(s);
        std::vector<tensor_input> in = {
            { "a", packed.data(), packed.size() },
    ...
    // (c) prefill: n=64 -> dequant+cuBLAS path (MMQ not yet specialized for RQ3)
    {
        g_cur_mm = { 32, 64, 8 * QK };
        std::vector<float> f32_a, f32_b((size_t) g_cur_mm.k * g_cur_mm.n);
        auto packed = make_rq3(g_cur_mm.m, g_cur_mm.k, 0x9ABCu, f32_a);
        uint32_t s = 0xDEF0u;
        for (auto & v : f32_b) v = frand(s);
        std::vector<tensor_input> in = {
            { "a", packed.data(), packed.size() },
        ...
        auto cpu  = run_graph_float(be_cpu,  build_mul_mat, in, "out");
        auto cuda = run_graph_float(be_cuda, build_mul_mat, in, "out");
        ok = diff(cpu, cuda, "VECDOT mul_mat n=64 (prefill dequant+cuBLAS)", 1.0, 5e-3) && ok;
    }

    // (d) DEQUANT via get_rows: CUDA k_get_rows_rq3 vs CPU dequantize_row_rq3
    {
        g_gr = { 64 * QK, 8 };
        std::vector<float> f32_a;
        auto packed = make_rq3(g_gr.nrow, g_gr.ncol, 0x5A5Au, f32_a);
        ...
        // get_rows dequant is an exact transform (no q8 re-quant): bit-tight bar.
