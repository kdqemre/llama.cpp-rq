...
// Mirrors rq4-prep.cu: identical math (same 32-point RHT + q8_1 quant + FP32 Sa
// sidecar), only the sign source differs (ggml_cuda_rq2_signs_dev).
...
// ---------------------------------------------------------------------------
// RQ2 MMQ activation rotation. Fused copy + forward randomized Hadamard
...
// <w_rot, RHT_forward(a)>.
// ---------------------------------------------------------------------------
static __global__ void rq2_copy_rotate_act_kernel(
    ...
    if (base >= n) return;
    float val = src[base + lane] * ggml_cuda_rq2_signs_dev[lane];
#pragma unroll
...
void ggml_cuda_rq2_rotate_act(
    ...
    rq2_copy_rotate_act_kernel<<<n_blocks, 32*n_warps, 0, stream>>>(dst, src, n);
}

// ---------------------------------------------------------------------------
// RQ2 fused activation prep (Kernel A): replaces memcpy + rotate + quantize
// with a single launch. Per 32-element sub-block (== one q8_1 block == one
// warp) it: 1. warp-reduces Sa = sum of the ORIGINAL activation, 2. applies the
// forward randomized Hadamard transform in-register, 3. q8_1-quantizes the
...
void ggml_cuda_rq2_prep_act(
    ...
    constexpr int prep_block_size = 256;
