...
// See rq4-prep.cuh for the design rationale. Extracted verbatim (minus the
// unrelated tq3_0 code) from tq3-native.cu in the source repo so the live
// 11-arg prep API + FP32 Sa sidecar matches what the RQ4 MMVQ/MMQ dispatch calls.
...
// ---------------------------------------------------------------------------
// RQ4 MMQ activation rotation. Fused copy + forward randomized Hadamard
...
// <w_rot, RHT_forward(a)> -> the WHT moves off the weights (load_tiles becomes a
// plain Q4_K nibble-unpack, zero runtime weight-side WHT) onto the activation,
// amortized once/token across all weight rows.
// ---------------------------------------------------------------------------
static __global__ void rq4_copy_rotate_act_kernel(
    ...
    if (base >= n) return;
    float val = src[base + lane] * ggml_cuda_rq4_signs_dev[lane];
#pragma unroll
...
void ggml_cuda_rq4_rotate_act(
    ...
    rq4_copy_rotate_act_kernel<<<n_blocks, 32*n_warps, 0, stream>>>(dst, src, n);
}

// ---------------------------------------------------------------------------
// RQ4 fused activation prep (Kernel A): replaces memcpy + rotate + quantize
...
static __global__ void rq4_prep_act_kernel(
    ...
    float val = av * ggml_cuda_rq4_signs_dev[lane];
#pragma unroll
    for (int step = 1; step < 32; step <<= 1) {
        const float other = __shfl_xor_sync(0xFFFFFFFF, val, step, 32);
        val = (lane & step) ? (other - val) : (other + val);
