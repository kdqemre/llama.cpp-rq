...
// moves off the weight-side load_tiles onto the activation, amortized
// once/token across all weight rows. Format-agnostic: identical math to the
// RQ4 prep, but reads the independent ggml_cuda_rq3_signs_dev sign source so
// the RQ3 sign search can be swept without disturbing RQ4.

#include "common.cuh"   // pulls ggml-common.h -> block_q8_1, QK8_1; cuda runtime

// RQ3 batched activation rotation for MMQ: fused copy + forward-WHT per
